...
```
├── internal/
│   ├── core/        # Core data structures (Player, Buff, Item, Event, Faction, Evaluation)
│   ├── engine/      # Game engine (Game, StateMachine, Handlers)
│   └── gamemap/     # Map system (Cell, MapEngine, PathResult)
├── pkg/
│   ├── event/       # EventBus system (Phase, Bus, Decision, Context)
│   ├── rng/         # Random number engine (WeightedPool, LuckModifier, EventPool, ItemPool)
...
- **Phase**: Trigger timing enumeration (BeforeTurn, OnMove, OnLand, PreEvent, PreDamage, AfterTurn, AnyTime, OnBuffApplied, OnBuffRemoved)
...
- **Player**: User entity with HP/LP/Buffs/Items/Metadata
...
- **StateMachine**: Phase trigger and user decision handling
...
1. **BeforeTurn**: Trigger BeforeTurn phase effects (神眷/诅咒 LP±1, 离火 every 4 turns)
2. **MainAction**: Player can use items or faction skills
3. **OnMove**: Roll dice, calculate path, handle Fragile/Fog
4. **OnLand**: Trigger landing events
5. **PreEvent**: Check event immunity (辟邪)
6. **AfterTurn**: Tick Buff durations, trigger AfterTurn effects
...
| 迷途 (Lost) | OnMove | Reverse movement direction |
