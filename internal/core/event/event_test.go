...
func TestEventTypeGetEvaluation(t *testing.T) {
	...
	goodTests := []struct {
		...
		{EventTypeHerb, types.EvaluationMildGood},
		{EventTypeMilkTea, types.EvaluationGood},
		{EventTypeRelic, types.EvaluationVeryGood},
		{EventTypeDivineBless, types.EvaluationExcellent},
	}
	...
	neutralTests := []struct {
		...
		{EventTypeExchange, types.EvaluationNeutral},
		{EventTypeHiddenBuff, types.EvaluationGood},
		{EventTypeTasteTest, types.EvaluationMixed},
	}
	...
	badTests := []struct {
		...
		{EventTypeMosquito, types.EvaluationMildBad},
		{EventTypeGhostHit, types.EvaluationMildBad},
		{EventTypeDogPoop, types.EvaluationMildBad},
		{EventTypeThief, types.EvaluationBad},
		{EventTypeCurseBuddha, types.EvaluationBad},
		{EventTypeLostWay, types.EvaluationMildBad},
		{EventTypeThunder, types.EvaluationVeryBad},
	}
...
func TestEventTypeGetEventDefinition(t *testing.T) {
	...
	if def.Eval != types.EvaluationMildGood {
		t.Errorf("def.Eval = %d, expected MildGood(%d)", def.Eval, types.EvaluationMildGood)
	}
	if def.HPChange != 1 {
	...
	if def.Eval != types.EvaluationVeryBad {
		t.Errorf("def.Eval = %d, expected VeryBad(%d)", def.Eval, types.EvaluationVeryBad)
	}
	if def.HPChange != -999 {
	...
	if def.BuffType != buff.BuffTypeDivine {
	...
	if def.SpecialEffect != types.SpecialDrawItem {
...
func TestEventRegistryEvaluationDistribution(t *testing.T) {
	...
	for _, et := range GetAllEventTypes() {
		eval := GetEventEvaluation(et)
		if eval <= types.EvaluationBadThreshold {
			bad++
		} else if eval <= types.EvaluationNeutralThreshold {
			neutral++
		} else {
			good++
		}
